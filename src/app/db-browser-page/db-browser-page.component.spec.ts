...
describe('DbBrowserPageComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      declarations: [ DbBrowserPageComponent ],
      ...
      providers: [
        ...
        { provide: DbConfigService, useValue: jasmine.createSpyObj('DbConfigService', ['getByName']) },

        DbTableSchemaProviderFactory,
        SampleDbTableSchemaProvider,
        { provide: NotifyService, useValue: jasmine.createSpyObj('NotifyService', ['error', 'success', 'warning'])}
      ]
    })
    .compileComponents();

    dbConfigServiceSpy = TestBed.inject(DbConfigService) as jasmine.SpyObj<DbConfigService>;
    paramMapSpy = TestBed.inject(ActivatedRoute).snapshot.paramMap as jasmine.SpyObj<ParamMap>;
    notifySpy = TestBed.inject(NotifyService) as jasmine.SpyObj<NotifyService>;
  });
  ...
  it('should show error message when route parameter not provided', fakeAsync(() => {
    ...
    expect(notifySpy.error).toHaveBeenCalledWith("Please provide a valid database config to open.");
  }));
  ...
  it('should show error message when config not found', fakeAsync(() => {
    ...
    expect(notifySpy.error).toHaveBeenCalledWith("Unable to locate config 'testing'.");
  }));
